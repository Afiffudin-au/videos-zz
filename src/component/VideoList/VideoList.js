...
function VideoList() {
  ...
  const handleControlNext = ()=>{
    ...
    getVideoList(videoListData?.nextPageToken)
  }
  const handleControlPrev = ()=>{
    ...
    getVideoList(videoListData?.prevPageToken)
  }
  return (
    <div className="VideoList p-1">
      ...
      <MemoizedChildComponent videoListData={videoListData}/>
      <div className="VideoList__control flex flex-col mt-2 mx-auto xs:w-8/12 sm:w-6/12">
        {
          videoListData?.prevPageToken && <button className="px-2 font-semibold py-2 mb-2 w-full border-none cursor-pointer hover:bg-red-700 text-lg rounded shadow-xl outline-none bg-red-500 text-black" onClick={handleControlPrev}>Prev Load More</button>
        }
        {
          videoListData?.nextPageToken && <button className="px-2 font-semibold py-2 w-full border-none cursor-pointer hover:bg-blue-700 text-lg rounded shadow-xl outline-none bg-blue-600 text-white" onClick={handleControlNext}>Next Load More</button>
        }
      </div>
    </div>
  )
}
function ChildComponent({videoListData}){
  return (
    <div className="VideoList__grid gap-2 mt-2 grid xs:grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {
       videoListData?.items?.map((item,index)=>(
